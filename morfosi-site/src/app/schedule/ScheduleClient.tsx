...
const GYMNASIO_YEARS = ["A_GYMNASIOU", "B_GYMNASIOU", "C_GYMNASIOU"];
const LYKEIO_YEARS = ["A_LYKEIOU", "B_LYKEIOU", "C_LYKEIOU"];
...
export default function ScheduleClient({
  ...
  const filteredYears =
    activeTab === "gymnasio"
      ? GYMNASIO_YEARS.filter((y) => displayClasses.some((c) => c.schoolYear === y))
      : LYKEIO_YEARS.filter((y) => displayClasses.some((c) => c.schoolYear === y));
  ...
  const filteredClasses = displayClasses.filter((c) =>
    activeTab === "gymnasio"
      ? GYMNASIO_YEARS.includes(c.schoolYear)
      : LYKEIO_YEARS.includes(c.schoolYear)
  );
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-6 lg:px-12 py-16 space-y-20">
        ...
        {filteredClasses.length === 0 && (
          <div className="text-center py-24 border-2 border-dashed border-gray-300 bg-white">
            <p className="text-gray-400 font-black text-lg uppercase tracking-widest">
              Δεν υπάρχουν τμήματα {activeTab === "gymnasio" ? "Γυμνασίου" : "Λυκείου"} ακόμα
            </p>
