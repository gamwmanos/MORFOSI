...
function TabButton({ active, onClick, title, color }: { active: boolean, onClick: () => void, title: string, color: string }) {
  return (
    <button 
      onClick={onClick}
      className={`relative px-6 py-4 font-black uppercase tracking-widest text-sm transition-all overflow-hidden ${
        active ? `${color} text-white` : 'bg-transparent text-gray-500 hover:text-gray-900 hover:bg-gray-200'
      }`}
    >
